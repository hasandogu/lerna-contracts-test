...
contract("ERC721OrderinboxPrivate", accounts => {

  let token;
  let tokenOwner = accounts[9];
  const name = 'FreeMintableOrderinboxPrivate';
  ...
  beforeEach(async () => {
    ...
    await token.__ERC721OrderinboxPrivate_init(name, "OIX", "https://ipfs.orderinbox.com", "https://ipfs.orderinbox.com", [whiteListProxy], [], {from: tokenOwner});
  });
  ...
  it("mint and transfer by whitelist proxy. minter is tokenOwner", async () => {
    ...
    await token.mintAndTransfer([tokenURI, creators([minter]), fees, [signature]], transferTo, {from: whiteListProxy});
  ...
  it("mint and transfer by whitelist proxy. minter is not tokenOwner", async () => {
    ...
    await expectThrow(
      token.mintAndTransfer([tokenURI, creators([minter]), fees, [signature]], transferTo, {from: whiteListProxy})
    );
  });
  ...
  it("mint and transfer by minter. minter is not tokenOwner", async () => {
    ...
    const tokenURI = "//uri";
    await expectThrow(
      token.mintAndTransfer([tokenURI, creators([minter]), [], [zeroWord]], transferTo, {from: minter})
    );
  });
  ...
  function getSignature(tokenURI, fees, creators, account) {
		return signAutoId(account, tokenURI, fees, creators, token.address);
  }
