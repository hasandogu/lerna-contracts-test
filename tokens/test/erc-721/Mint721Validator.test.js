...
const { sign } = require("../../../common/test/mint");
...
contract("Mint721Validator", accounts => {
	...
	it("should fail if signer is incorrect", async () => {
		...
		await expectThrow(
			testing.validateTest([1, "testURI", creators, fees, [signature], false], 0)
		);
	});
	...
	it("should validate if signer is contract and 1271 passes", async () => {
		...
		await expectThrow(
			testing.validateTest([1, "testURI", creators, fees, ["0x"], false], 0)
		);

		await erc1271.setReturnSuccessfulValidSignature(true);

		await testing.validateTest([1, "testURI", creators, fees, ["0x"], false], 0)
	});
